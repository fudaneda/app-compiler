...
std::vector<std::map<int, Loop*>> getMappingUnitNameUsingLoopMark(Function &F, std::map<std::string, MappingUnit> &mappingUnitMap,
						...
						std::vector<Instruction *> &outMarkLoopIns, std::pair<BasicBlock*, BasicBlock*> &LoopIOBBs)
{
	...
	LoopIOBBs = std::make_pair(EntryBB, ExitBB);
...
namespace {
    ...
    struct CDFGPass : public FunctionPass {
	    ...
	    virtual bool runOnFunction(Function &F) {
			...
			std::vector<Instruction*> outMarkLoopIns;
			std::pair<BasicBlock*, BasicBlock*> LoopIOBBs;
            auto LoopVec = getMappingUnitNameUsingLoopMark(F, MappingUnitMap, SuccBBsMap, outMarkLoopIns, LoopIOBBs);//check LoopMark first
			if(LoopVec.empty()){
			...
			else
			{
				...
				{	
					...
					CDFG->setBackEdgeBBPs(BackEdgeBBPs);
					CDFG->setSuccBBMap(SuccBBsMap);
					CDFG->setLoopBBs(OutterMostUnit.allBlocks, OutterMostUnit.entryBlocks, OutterMostUnit.exitBlocks, LoopIOBBs);
					CDFG->setLoops(nestloops);
					CDFG->setFuncArgs(funcArgs);
					CDFG->setExInsList(outMarkLoopIns);
					// CDFG->LoopIdxAnalyze();
					CDFG->initialize();
					// CDFG->accessAnalyze();
