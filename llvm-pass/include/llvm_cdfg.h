...
private:
    ...
    //the BB where the loop_begin exist(may be not included in _loopentryBBs when loopsafetycheck)
    BasicBlock* _loopbeginBB;
    //the BB where the loop_end exist(may be not included in _loopentryBBs when loopsafetycheck)
    BasicBlock* _loopendBB;
...
///TODO:somtimes may have more than one PHINode in a level
    ...
    void setLoopBBs(std::set<BasicBlock*> BBs, std::set<std::pair<BasicBlock*,BasicBlock*>> entryBBs, std::set<std::pair<BasicBlock*,BasicBlock*>> exitBBs, std::pair<BasicBlock*,BasicBlock*> LoopIOBBs){
        _loopBBs = BBs; 
        _loopentryBBs = entryBBs; 
        _loopexitBBs = exitBBs;
        _loopbeginBB = LoopIOBBs.first;
        _loopendBB = LoopIOBBs.second;
    }
